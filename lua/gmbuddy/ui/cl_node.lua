...
function PANEL:Init()
	...
	self.Label.SetSelected = function(s, b) self.m_bSelected = b end
	self.Label.Paint = function(s, w, h)
		if self.m_bSelected then
			draw.RoundedBox(0, 0, 0, w, h, cfg.Colors["Selected"])
		end
	end
	self.Label:SetTextInset(128, 0)
	self.Label:SetFont("GMB_UI")
...
function PANEL:SetExpanded( bExpand, bSurpressAnimation )
	...
	if ( !bSurpressAnimation ) then
		self.animSlide:Start( 0.1, { From = StartTall } )
		self.animSlide:Run()
	end
...
function PANEL:PerformLayout()
	...
	if ( self:ShowIcons() ) then
		...
		self.Icon:SetPos( self.Expander.x + self.Expander:GetWide() + 4, ( LineHeight - self.Icon:GetTall() ) * 0.4 )
...
function PANEL:AddNode( strName, strIcon )
	...
	local pNode = vgui.Create( "GMBTree_Node", self )
...
function PANEL:Copy()
	...
	local copy = vgui.Create( "GMB_Tree_Node", self:GetParent() )
...
derma.DefineControl( "GMBTree_Node", "Tree Node", PANEL, "DPanel" )