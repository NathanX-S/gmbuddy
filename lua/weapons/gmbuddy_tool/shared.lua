...
SWEP.PrintName		= "GMBuddy Toolgun"
SWEP.Author			= "Eris"
SWEP.Contact		= ""
SWEP.Purpose		= ""
SWEP.Instructions	= ""
SWEP.Category = "GMBuddy"
SWEP.Spawnable = true
...
function SWEP:Initialize()
	...
	self.NextMouse3 = CurTime()
	self.Mouse3Released = true
...
function SWEP:Think()
	...
	if CLIENT then
		if (!input.IsMouseDown(MOUSE_MIDDLE) && !self.Mouse3Released) then
			self.Mouse3Released = true
		end

		if (input.IsMouseDown(MOUSE_MIDDLE) && CurTime() > self.NextMouse3 && self.Mouse3Released) then
			self.Mouse3Released = false
...
function SWEP:DoShootEffect( hitpos, hitnormal, entity, physbone, bFirstTimePredicted )
	...
	if (self:GetOwner():GetNWBool("ToolgunStealth") && SERVER) then return end
...
function SWEP:Reload()
	...
	-- Ask the gamemode if it's ok to do this
	local mode = self:GetMode()
	if ( !gamemode.Call( "CanTool", self:GetOwner(), trace, mode, tool, 3 ) ) then return end
