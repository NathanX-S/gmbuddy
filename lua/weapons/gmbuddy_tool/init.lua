...
function GMBuddy.Tool( ply, command, arguments )
	...
	local isTool = ( IsValid( activeWep ) && (activeWep:GetClass() == "gmbuddy_tool" || activeWep:GetClass() == "gmod_tool") )

	-- Switch weapons
	ply:SelectWeapon( "gmbuddy_tool" )

	-- Get the weapon and send a fake deploy command
	local wep = ply:GetWeapon( "gmbuddy_tool" )
...
concommand.Add( "gmod_tool", GMBuddy.Tool, nil, nil, { FCVAR_SERVER_CAN_EXECUTE } )